...
### Fixed
- Expired ADC no longer requires an MCP server restart. On `RefreshError`/`Unauthenticated`/401 the dispatcher rebuilds the BigQuery client from the ADC file on disk and retries the call once.

## [0.1.3] — 2026-07-29

### Fixed
- Pin `mcp` SDK to `<2.0.0` to fix `uvx` breakage from the 2.0 release (`Server` object lost `list_tools`).
